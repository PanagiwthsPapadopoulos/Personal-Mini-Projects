...
int main(){
    ...
    int cl, coefficient[6], res, root;        //συντελεστες του χ
...
int equation(int coefficient[]){                                                                 // identify the class and the equation itself
    ...
    switch(x){
        case 5:
        printf("\nEnter the coefficient of the fifth power: ");
        scanf("%d", &coefficient[5]);
        printf("\nEnter the coefficient of the fourth power: ");
        scanf("%d", &coefficient[4]);
    ...
    switch(x){
        case 5:
        printf("The equation is: (%d)x^5 + (%d)x^4 + (%d)x^3 + (%d)x^2 + (%d)x + (%d) = 0 \n", coefficient[5], coefficient[4], coefficient[3], coefficient[2], coefficient[1], coefficient[0]);
        break;
        case 4:
        printf("The equation is: (%d)x^4 + (%d)x^3 + (%d)x^2 + (%d)x + (%d) = 0 \n", coefficient[4], coefficient[3], coefficient[2], coefficient[1], coefficient[0]);
        break;
        case 3:
...
int values(int coefficient[], double space[], double value[], int cl){                 //calculates sign of the two different values
    ...
    first = ( pow(space[0], 5) * coefficient[5] ) + ( pow(space[0], 4) * coefficient[4] ) + ( pow(space[0], 3) * coefficient[3] ) + ( pow(space[0], 2) * coefficient[2] ) + ( space[0] * coefficient[1] ) + ( coefficient[0] );
    second = ( pow(space[1], 5) * coefficient[5] ) + ( pow(space[1], 4) * coefficient[4] ) + ( pow(space[1], 3) * coefficient[3] ) + ( pow(space[1], 2) * coefficient[2] ) + ( space[1] * coefficient[1] ) + ( coefficient[0] );
...
int roots(int coefficient[], double space[], double value[], int cl){                 //determines which part of the space the root is located.
    ...
    middle = ( pow(space[2], 5) * coefficient[5] ) + ( pow(space[2], 4) * coefficient[4] ) + ( pow(space[2], 3) * coefficient[3] ) + ( pow(space[2], 2) * coefficient[2] ) + ( space[2] * coefficient[1] ) + ( coefficient[0] );
